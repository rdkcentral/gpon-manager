    ...
    switch ( command )
    {
        case    'e' :
...

            {
                ...
                if ( g_Subsystem[0] != 0 )
                {
                    _ansc_sprintf(CName, "%s%s", g_Subsystem, COMPONENT_ID_GPON_MANAGER);
                }
                else
                {
                    _ansc_sprintf(CName, "%s", COMPONENT_ID_GPON_MANAGER);
                }
...
int main(int argc, char* argv[])
{
    ...
    pComponentName          = COMPONENT_NAME_GPON_MANAGER;
    ...
    fd = fopen("/var/tmp/gponmanager.pid", "w+");
    if ( !fd )
    {
    ...
    sd_notifyf(0, "READY=1\n"
              "STATUS=gponmanager is Successfully Initialized\n"
              "MAINPID=%lu", (unsigned long) getpid());

    CcspTraceInfo(("RDKB_SYSTEM_BOOT_UP_LOG : gponmanager sd_notify Called\n"));
#endif

    system("touch /tmp/GponMgrDml_Initialized");
